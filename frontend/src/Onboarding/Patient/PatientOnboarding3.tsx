...
const PatientOnboarding3: React.FC = () => {
  ...
  const [errors, setErrors] = useState<{ allergies?: string; medicalHistory?: string }>({});

  const validate = () => {
    const errs: typeof errors = {};
    if (!allergiesInput.trim()) errs.allergies = "Please enter allergies";
    if (!medicalHistoryInput.trim()) errs.medicalHistory = "Please enter medical history";
    setErrors(errs);
  ...
  return (
    <div className="relative w-full min-h-screen bg-white flex flex-col items-start p-8 overflow-hidden">
      ...
      <div className="z-10 w-full max-w-lg mx-auto mt-32 flex flex-col gap-6">
        <p className="text-xl md:text-2xl font-semibold text-gray-700">Medical Info</p>
        ...
        <div className="flex flex-col">
          ...
          />
          {errors.allergies && <span className="text-sm text-red-500 mt-1">{errors.allergies}</span>}
        </div>
        ...
        <div className="flex flex-col">
          ...
          />
          {errors.medicalHistory && <span className="text-sm text-red-500 mt-1">{errors.medicalHistory}</span>}
        </div>
